...
现在我们要筛选出所有 `IU` 台网的宽频带地震台站，则可以在 **Network** 框中输入 `IU`，
在 **Channel** 框中输入 `BH?`（此处的问号为通配符）。点击上方的 **Update Map**
按钮，则界面右侧会显示出所有满足筛选条件的台站。右侧上方为地图区域，圆圈标记了
...
更进一步，可以查看每个台站的详细信息。以位于美国新墨西哥州的地震台站 `IU.ANMO` 为例。
点击 `IU.ANMO` 台站对应的圆圈，会出现如下图所示的提示框：

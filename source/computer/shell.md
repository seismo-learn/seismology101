...
:::{div} page-meta
贡献者：{{姚家园}}、{{田冬冬}} · 最近更新：2023-04-08 · 预计花费时间：60 分钟
:::
...
假如当前目录下有 `filea.txt`、`fileb.txt` 和 `fileab.txt` 三个文件。
在命令中使用 `file?.txt` 时，Shell 会对 `file?.txt` 进行扩展。由于字符 `?` 可以
匹配任意单个字符，因而 `file?.txt` 会匹配到文件 `filea.txt` 和 `fileb.txt`：
```
$ ls file?.txt
filea.txt fileb.txt
```
同理，`file??.txt` 可以匹配文件 `fileab.txt`：
```
$ ls file??.txt
fileab.txt
```
...
假如当前目录下有 `filea.txt`、`fileb.txt` 和 `filec.txt` 三个文件。
使用 `file[ab].txt` 则会匹配到 `filea.txt` 和 `fileb.txt` 两个文件：
```
$ ls file[ab].txt
filea.txt fileb.txt
```
使用 `file[cd].txt` 则只能匹配到 `filec.txt` 这一个文件：
```
$ ls file[cd].txt
filec.txt
```
...
若当前目录下存在文件 `filea.txt`、`fileaaa.txt`、`filebbb.txt`
和 `fileabc.txt`，则通配符 `file[a-b]b?.txt` 会匹配到 `filebbb.txt`
和 `fileabc.txt` 两个文件：
```
$ ls file[a-b]b?.txt
...
`/dev/null` 文件。此文件是系统设备，叫做位存储桶，可以接受输入，并且
对输入不做任何处理。例如:

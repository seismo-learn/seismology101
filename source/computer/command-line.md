...
其中，`ls` 是命令名，`-l` 是选项，用于控制命令的行为，`/home` 是参数，即命令作用的
对象。选项通常以 `-` 或 `--` 开头，分别称为短选项和长选项，例如 `ls -a` 与
`ls --all` 的作用相同。多个短选项可以合并书写，例如 `ls -la` 等价于 `ls -l -a`。
需要注意，选项区分大小写，例如 `ls -r`（反向排序）与 `ls -R`（递归列出）的含义并不相同。

命令名、选项和参数之间用空格分隔。如果参数本身包含空格，需要将其用引号括起来，
例如 `mkdir "my folder"`。

:::{tip}
1. 使用向上、向下箭头按键可以获取之前输入的命令
2. 命令正在运行且无法结束时，按下 {kbd}`Ctrl` + {kbd}`C` 键可以中断该命令
3. 在 Linux 下，按住鼠标左键拖动选中文本，或双击选中一个单词，即可复制；随后按下
   鼠标中键，就可以将文本粘贴到光标所在的位置。在 macOS 下则使用 {kbd}`Command` +
   {kbd}`C` 复制、{kbd}`Command` + {kbd}`V` 粘贴
4. 输入命令或路径时按下 {kbd}`Tab` 键可自动补全
:::

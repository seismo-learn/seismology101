...
用户可以随意对家目录下的文件进行任何读、写、删除等操作，但却无法对根目录下的
文件进行类似操作。这是因为 Linux 下有严格的权限机制，以防止一般用户的操作对
系统造成破坏。有些时候，为了安装软件或者修改系统配置文件，则需要临时获取
root 用户（即管理员）权限。此时可以在需要执行的命令前加上 ``sudo``。
例如，在 Fedora 下安装 GNU Fortran::

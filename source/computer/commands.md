...
:::{div} page-meta
贡献者：{{姚家园}}、{{田冬冬}} · 最近更新：2021-04-04 · 预计花费时间：120 分钟
:::
...
使用 `diff` 命令比较 `file1` 和 `file2` 的异同。结果显示，两个文件
第二行不同，其他行是相同的:
```
$ diff file1 file2
2c2
...
从输出中可以得知，`/home` 分区总硬盘空间为 1.1 TB，已使用 904 GB，剩余 149 GB。
...
`locate` 命令可以用于查找目录和文件。该命令比 [find](#find) 命令快得多，原因在于它
...
与其他文件传输工具（如 [scp](#scp)）不同，`rsync` 命令仅传输有变动的部分。因此，
同步速度更快，常用于文件备份。可以参考
[SSH 教程：rsync 命令](https://wangdoc.com/ssh/rsync.html)
进一步学习其用法。

以下示例假设源目录是 `~/Downloads/source`，目标目录是 `~/workspace/destination`，
...
和远程计算机之间传输文件。该命令基于 [ssh](#ssh) 进行安全的远程文件传输，因此传输是加密的。
虽然 `scp` 传输速度不如 [rsync](#rsync) 命令，但是它不占系统资源。当需要传输大量小文件时，
使用 [rsync](#rsync) 命名会导致硬盘 I/O（输入/输出）非常高，而 `scp` 基本不影响系统
正常使用。可以参考
...
以上查看和解压命令也适用于 `.tar` 和 `tar.bz2` 压缩包格式。
...
如果重复行不相邻，则该命令不起作用。所以，`uniq` 命令一般与 [sort](#sort) 命令结合使用。
...
若重复行在文件中不相邻，可以使用 [sort](#sort) 命令先对文件进行排序:
...
从输出中可以得知，`/etc/passwd` 有 119 行、321 个单词以及 7579 个字节。

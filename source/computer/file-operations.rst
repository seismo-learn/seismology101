...
且属于用户组 seismo-learn（对于个人电脑而言，通常用户组下有且仅有一个用户，
因而用户组与用户同名）。

第一列文件权限位总共包含了13位信息（如 ``-rw-r--r--``\ ），从左到右意义分别是：

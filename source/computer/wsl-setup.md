...
:::{div} page-meta
贡献者：{{赵志远}}、{{田冬冬}}、{{姚家园}} · 最近更新：2025-10-11 · 预计花费时间：120 分钟
:::
...
1.  在 Windows 系统的搜索框中输入 `PowerShell` 或 `cmd`
...
[官方安装指南](https://learn.microsoft.com/zh-cn/windows/wsl/install)。
...
这样做的另一个好处是，导入后 Linux 发行版的数据文件会位于用户指定的目录中，
便于管理和备份。打开 CMD，执行如下命令:
```
# 导出 Linux 发行版，可做为备份
...
推荐使用 [Windows Terminal](https://learn.microsoft.com/zh-cn/windows/terminal/)，

...
:::{div} page-meta
贡献者：{{田冬冬}}、{{姚家园}} · 最近更新：2021-01-21 · 预计阅读时间：20 分钟
:::
...
`echo $SHELL`，查看当前 Shell，会显示 `/bin/zsh`。
...
Zsh 的配置文件为 `~/.zshrc`。因而切换到 Zsh 后，
所有的 Shell 配置都不用写到 `~/.bashrc`，而要写到 `~/.zshrc` 中。
...
该命令会创建 `~/.oh-my-zsh` 目录，下载 Oh My Zsh 到该目录下，生成默认的 Zsh 配置文件
`~/.zshrc`，并备份老的配置到类似 `~/.zshrc.pre-oh-my-zsh` 的文件中。
...
之后可以根据个人习惯修改配置文件 `~/.zshrc`，如[设置主题](https://github.com/ohmyzsh/ohmyzsh#themes)、[启用插件](https://github.com/ohmyzsh/ohmyzsh#plugins) 等。

### 设置主题

修改配置文件 `~/.zshrc` 中的变量 **ZSH_THEME** 即可使用不同的主题。例如，
可以设置:
...
Oh My Zsh 自带了很多主题，位于 `~/.oh-my-zsh/themes` 目录下，可以在线[预览主题效果](https://github.com/ohmyzsh/ohmyzsh/wiki/Themes)。
...
Oh My Zsh 自带了很多插件，位于 `~/.oh-my-zsh/plugins` 目录下，也可以[在线查询](https://github.com/ohmyzsh/ohmyzsh/wiki/Plugins-Overview)。
这里推荐几个常用的自带插件。

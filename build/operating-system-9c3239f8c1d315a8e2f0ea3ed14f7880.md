...
Microsoft Windows 用户数目远远多于 macOS 和 Linux 用户。但对地震学科研工作而言，
Linux 和 macOS 往往更适合作为日常科研的主要操作系统。主要原因有：

1. Linux/macOS 上更容易获取和安装科研工具链，如编译器、Python 等
2. 大多数地震学相关的程序/代码都是在 Linux/macOS 上编写的，且通常优先支持 Linux/macOS
3. Linux/macOS 自带了常用的命令行工具，比如 `grep`、`sed`、`awk`
4. 高性能计算服务器几乎都只使用 Linux 系统，因此在本机使用类似环境更方便迁移工作流
...
Linux 有[上百个不同的发行版](https://distrowatch.com/)，绝大多数都可以免费获取与安装。
比较流行的发行版有 [Ubuntu](https://ubuntu.com/)、[Debian](https://www.debian.org/)、
...
如前所述，Windows 通常不是地震学科研工作的首选系统。对于日常办公依赖 Windows、但科研工作
又需要 Linux 环境的情况，通常有如下几种解决办法：
...
**对于 Windows 10/11 用户，推荐使用 WSL**。它是微软提供的在 Windows 上运行 Linux 二进制的
兼容层，通过安装主流 Linux 发行版，可以原生运行大多数命令行工具和科研软件。
